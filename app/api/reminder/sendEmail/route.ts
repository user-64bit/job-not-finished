...
export async function GET(request: NextRequest) {
  console.log(
    `[Cron Job] Weekly reminder triggered at ${new Date().toISOString()}`,
  );
  ...
  try {
    ...
    console.log(
      `[Cron Job] Found ${users.length} total users, ${usersWithProjects.length} with reminder-enabled projects`,
    );

    if (usersWithProjects.length === 0) {
      console.log(
        "[Cron Job] No users have reminder-enabled projects. Exiting.",
      );
      return NextResponse.json({
        success: true,
    ...
    for (const user of usersWithProjects) {
      ...
      console.log(
        `[Cron Job] Processing user ${user.id} with ${user.projects.length} reminder-enabled projects`,
      );
      ...
      for (const project of user.projects) {
        console.log(
          `[Cron Job] Processing project ${project.id}: ${project.name}`,
        );
        ...
        if (repoInfo) {
          ...
          projectRoastsPromises.push(
            generateProjectRoast({
              name: project.name,
              owner: user.githubId,
            }),
          );
      ...
      const projectRoastsHtml = projectRoasts
        .map(
          (project: ProjectRoast) => `
      ...
      emailPromises.push(
        resend.emails
          ...
          .then((response) => {
            console.log(
              `[Cron Job] Email sent to ${user.email} with ID: ${response.data?.id}`,
            );
            return {
              userId: user.id,
              email: user.email,
          ...
          .catch((error) => {
            console.error(
              `[Cron Job] Failed to send email to ${user.email}:`,
              error,
            );
            return {
              userId: user.id,
    ...
    if (emailPromises.length === 0) {
      console.log(
        "[Cron Job] No emails were queued for sending, something may be wrong with the filtering",
      );
      return NextResponse.json({
        success: true,
    ...
    console.log(
      `[Cron Job] Email batch completed. Success: ${successCount}, Failed: ${failureCount}`,
    );
